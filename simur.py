...
def parse_loewdin_mo_pops(path: str) -> dict:
    ...
    while pos < len(sec_lines):
        ...
        while pos < len(sec_lines):
            ...
            # Determine angular momentum type from label.  ORCA usually prints
            # resolved labels (px, dz2, ...), but tolerate shell-prefixed forms.
            ang_resolved = re.sub(r"^\d+", "", ang_lbl).lower()
            ...
            for k in range(min(len(vals), n_mos)):
                ...
                if atom_key not in cur[mo_i]["atoms"]:
                    cur[mo_i]["atoms"][atom_key] = {
                        "s": 0.0, "p": 0.0, "d": 0.0, "f_orb": 0.0,
                        "total": 0.0, "ao": {}
                    }
                cur[mo_i]["atoms"][atom_key][ang_type] += pct
                cur[mo_i]["atoms"][atom_key]["total"]  += pct
                ao = cur[mo_i]["atoms"][atom_key]["ao"]
                ao[ang_resolved] = ao.get(ang_resolved, 0.0) + pct
...
def compute_mo_composition(mo_data: dict, mo_index: int,
    ...
    if method not in {"auto", "reduced", "raw"}:
        method = "auto"
    reduced_entry = (mo_data.get("reduced_populations") or {}).get(mo_index)
    requested_reduced = method in {"auto", "reduced"}
    use_reduced = requested_reduced and bool(reduced_entry)
    ...
    pct_weights = 100.0 * raw_w / total_raw

    if use_reduced:
        reduced_atoms = reduced_entry.get("atoms", {})
        raw_group_totals = {}
        for i, (a_idx, a_el, shell_n, ang_lbl) in enumerate(ao_labels):
            ...
            if ang_resolved.startswith('d'):
                ...
                group = 's'
            key = (atom_key, group)
            raw_group_totals[key] = raw_group_totals.get(key, 0.0) + pct_weights[i]

        corrected = np.zeros_like(pct_weights)
        for i, (a_idx, a_el, shell_n, ang_lbl) in enumerate(ao_labels):
            atom_key = f"{a_el}{a_idx + 1}"
            atom_pop = reduced_atoms.get(atom_key)
            if not atom_pop:
                continue
            ang_resolved = _resolve_ang(ang_lbl)
            if ang_resolved.startswith('d'):
                ...
                group = 's'
            target_pct = 100.0 * atom_pop.get(group, 0.0)
            raw_group = raw_group_totals.get((atom_key, group), 0.0)
            if raw_group > 1e-30:
                corrected[i] = pct_weights[i] * target_pct / raw_group
        pct_weights = corrected
    ...
    return {
        'method': 'reduced' if use_reduced else 'raw',
        'method_label': 'Löwdin reduced population' if use_reduced else 'Raw coefficient squared',
        'method_warning': (
            "Did not detect Löwdin reduced populations; using raw C^2 instead."
            if requested_reduced and not use_reduced else ""
        ),
...
class SimurApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("simur")
        _set_window_icon(self, "Simur.png")
    ...
    def _build_sidebar(self, p):
        ...
        tk.Button(btn_frame, text="Open File(s)…", bg="#1a3c6e", fg="black",
    ...
    def _build_tab_3d(self, p):
        ...
        measure_bar = tk.Frame(p)
        measure_bar.pack(fill=tk.X, padx=6, pady=(4, 0))
        tk.Label(measure_bar, text="Measure:", font=("", 8, "bold")).pack(side=tk.LEFT)
        for label in ("Label", "Distance", "Angle", "Dihedral"):
            ttk.Radiobutton(measure_bar, text=label, value=label,
                            variable=self._measure_mode,
                            command=self._on_measure_mode_changed).pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(measure_bar, text="Clear", command=self._clear_measurement).pack(side=tk.LEFT, padx=8)
        tk.Label(measure_bar, textvariable=self._measure_status,
                 fg="#cccccc", bg="#1c1c1e", anchor="w").pack(side=tk.LEFT, fill=tk.X, expand=True)
        view_frame = tk.Frame(p, bg="#1c1c1e")
        view_frame.pack(fill=tk.BOTH, expand=True)
        c3d = FigureCanvasTkAgg(self._fig_3d, master=view_frame)
        c3d.draw()
        c3d.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        self._build_measure_overlay(view_frame)
        self._toolbar_3d = NavigationToolbar2Tk(c3d, p)
        self._canvas_3d = c3d
        ...
        self._3d_bond_artists: Dict[object, Tuple[int, int]] = {}
        self._measure_atoms: List[int] = []
        self._measure_artists: List[object] = []
    ...
    def _build_measure_overlay(self, parent):
        ...
        panel.place(relx=1.0, rely=1.0, x=-14, y=-14, anchor="se")
        header = tk.Frame(panel, bg="#232a31", cursor="fleur")
        header.pack(fill=tk.X)
        tk.Label(header, text="Measurement", bg="#232a31", fg="white",
                 font=("", 8, "bold"), padx=6, pady=2).pack(side=tk.LEFT)
        tk.Button(header, text="x", font=("", 7), bd=0,
                  command=self._clear_measurement).pack(side=tk.RIGHT, padx=2, pady=1)
        tk.Label(panel, textvariable=self._measure_readout,
                 bg="#101317", fg="#d8fbff", font=("Courier", 9, "bold"),
                 justify=tk.LEFT, anchor="w", padx=8, pady=6,
                 width=38).pack(fill=tk.X)
        for widget in (panel, header):
            widget.bind("<ButtonPress-1>", self._measure_panel_begin_drag)
            widget.bind("<B1-Motion>", self._measure_panel_drag)
        self._measure_panel = panel
        self._measure_drag_offset = (0, 0)

    def _measure_panel_begin_drag(self, event):
        self._measure_drag_offset = (event.x_root - self._measure_panel.winfo_rootx(),
                                     event.y_root - self._measure_panel.winfo_rooty())

    def _measure_panel_drag(self, event):
        parent = self._measure_panel.master
        parent.update_idletasks()
        ox, oy = getattr(self, "_measure_drag_offset", (0, 0))
        x = event.x_root - parent.winfo_rootx() - ox
        y = event.y_root - parent.winfo_rooty() - oy
        max_x = max(parent.winfo_width() - self._measure_panel.winfo_width(), 0)
        max_y = max(parent.winfo_height() - self._measure_panel.winfo_height(), 0)
        self._measure_panel.place(x=max(0, min(x, max_x)),
                                  y=max(0, min(y, max_y)),
                                  anchor="nw")
    ...
    def _build_iso_cube_subpanel(self, p):
        ...
        tk.Button(btn, text="Load Cube(s)...", bg="#1a3c6e", fg="black",
    ...
    def _build_iso_mo_subpanel(self, p):
        ...
        tk.Button(btn, text="Try loading MOs from .out...", bg="#1a3c6e", fg="black",
    ...
    def _build_iso_render_controls(self, p):
        ...
        tk.Button(r4, text="  Render  ", bg="#2a4a00", fg="black",
        ...
        # Top summary line and population-analysis selector
        comp_top = tk.Frame(comp_frame)
        comp_top.pack(fill=tk.X)
        self._comp_summary = tk.Label(comp_top, text="Select and render an MO to see composition.",
                                      fg="gray", font=("", 8), anchor="w")
        self._comp_summary.pack(side=tk.LEFT, fill=tk.X, expand=True)

        tk.Label(comp_top, text="Analysis:", font=("", 8)).pack(side=tk.LEFT, padx=(6, 2))
        self._comp_method_combo = ttk.Combobox(
            comp_top, textvariable=self._comp_method_var,
            values=self._comp_method_choices, width=18, state="readonly",
            font=("", 8)
        )
        self._comp_method_combo.pack(side=tk.LEFT)
        self._comp_method_combo.bind("<<ComboboxSelected>>",
                                     lambda _e: self._refresh_current_mo_composition())
        tk.Button(comp_top, text="Next", font=("", 7), width=5,
                  command=self._cycle_comp_method).pack(side=tk.LEFT, padx=(3, 0))
    ...
    def _draw_3d(self, name: str, data: dict, highlighted: Optional[int] = None):
        ...
        if style != "Spacefill":
            ...
            for i, j in _detect_bonds(vis):
                line, = self._ax_3d.plot([xs[i], xs[j]], [ys[i], ys[j]], [zs[i], zs[j]],
                                         color=bclr, linewidth=lw, alpha=0.90,
                                         solid_capstyle="round")
                self._3d_bond_artists[line] = (i, j)

        sc = self._ax_3d.scatter(xs, ys, zs, c=colors, s=sizes,
                                  depthshade=True, alpha=0.92, picker=True)
        sc.set_pickradius(18)
    ...
    def _on_measure_mode_changed(self):
        self._clear_measurement(redraw=False)
        mode = self._measure_mode.get()
        if mode == "Label":
            msg = "Pick atoms to label them."
        elif mode == "Distance":
            msg = "Distance: click atom 1 of 2."
        elif mode == "Angle":
            msg = "Angle: click atom 1 of 3; atom 2 is the vertex."
        else:
            msg = "Dihedral: click atom 1 of 4."
        self._measure_status.set(msg)
        self._measure_readout.set(msg)
        self._canvas_3d.draw_idle()

    def _clear_measurement(self, redraw=True):
        for artist in getattr(self, "_measure_artists", []):
            try:
                artist.remove()
            except Exception:
                pass
        self._measure_artists = []
        self._measure_atoms = []
        if hasattr(self, "_measure_status"):
            mode = self._measure_mode.get()
            msg = "Pick atoms to label them." if mode == "Label" else f"{mode}: click atom 1."
            self._measure_status.set(msg)
            self._measure_readout.set("No measurement yet" if mode == "Label" else msg)
        if redraw and hasattr(self, "_canvas_3d"):
            self._canvas_3d.draw_idle()

    def _atom_point(self, idx: int) -> np.ndarray:
        x, y, z, _elem, _serial = self._3d_atom_data[idx]
        return np.array([x, y, z], dtype=float)

    def _atom_name(self, idx: int) -> str:
        _x, _y, _z, elem, serial = self._3d_atom_data[idx]
        return f"{elem}{serial}"

    def _angle_degrees(self, a: np.ndarray, b: np.ndarray, c: np.ndarray) -> float:
        v1 = a - b
        v2 = c - b
    ...
    def _dihedral_degrees(self, p0: np.ndarray, p1: np.ndarray,
    ...
    def _add_measure_marker(self, idx: int):
        p = self._atom_point(idx)
        marker = self._ax_3d.scatter([p[0]], [p[1]], [p[2]], s=360,
                                     facecolors="#d8fbff", edgecolors="#00e5ff",
                                     alpha=0.38, linewidths=2.6,
                                     depthshade=False, zorder=120)
        ring = self._ax_3d.scatter([p[0]], [p[1]], [p[2]], s=560,
                                   facecolors="none", edgecolors="#ffff66",
                                   linewidths=2.4, depthshade=False, zorder=121)
        label = self._ax_3d.text(p[0], p[1], p[2], f"  {self._atom_name(idx)}",
                                 color="#ffff66", fontsize=10, fontweight="bold",
                                 ha="left", va="center", zorder=122)
        self._measure_artists.extend([marker, ring, label])

    def _draw_measurement(self, indices: List[int], mode: str):
        pts = [self._atom_point(i) for i in indices]
        names = [self._atom_name(i) for i in indices]
        line_color = "#00e5ff"
        if mode == "Distance":
            value = float(np.linalg.norm(pts[1] - pts[0]))
            text = f"Distance\n{names[0]} - {names[1]}\n{value:.3f} A"
            scene_text = f"{names[0]}-{names[1]}: {value:.3f} A"
            segments = [(0, 1)]
            text_pos = (pts[0] + pts[1]) / 2.0
        elif mode == "Angle":
            value = self._angle_degrees(pts[0], pts[1], pts[2])
            text = f"Angle\n{names[0]} - {names[1]} - {names[2]}\n{value:.2f} deg"
            scene_text = f"{names[0]}-{names[1]}-{names[2]}: {value:.2f} deg"
            segments = [(0, 1), (1, 2)]
            text_pos = pts[1]
        else:
            value = self._dihedral_degrees(pts[0], pts[1], pts[2], pts[3])
            text = f"Dihedral\n{names[0]} - {names[1]} - {names[2]} - {names[3]}\n{value:.2f} deg"
            scene_text = f"{names[0]}-{names[1]}-{names[2]}-{names[3]}: {value:.2f} deg"
            segments = [(0, 1), (1, 2), (2, 3)]
            text_pos = sum(pts) / len(pts)

        for a, b in segments:
            line, = self._ax_3d.plot([pts[a][0], pts[b][0]],
                                     [pts[a][1], pts[b][1]],
                                     [pts[a][2], pts[b][2]],
                                     color=line_color, linewidth=5.0, alpha=0.98,
                                     solid_capstyle="round", zorder=119)
            self._measure_artists.append(line)
        ann = self._ax_3d.text(text_pos[0], text_pos[1], text_pos[2], f"  {scene_text}",
                               color="black", fontsize=9, fontweight="bold",
                               ha="left", va="center",
                               bbox=dict(boxstyle="round,pad=0.2",
                                         facecolor="#d8fbff", alpha=0.92,
                                         edgecolor=line_color, linewidth=0.8),
                               zorder=122)
        self._measure_artists.append(ann)
        self._measure_status.set(scene_text)
        self._measure_readout.set(text)
        if hasattr(self, "_status_var"):
            self._status_var.set(scene_text)

    def _handle_measure_pick(self, idx: int):
        mode = self._measure_mode.get()
        needed = {"Distance": 2, "Angle": 3, "Dihedral": 4}.get(mode)
        if not needed:
            return
        if len(getattr(self, "_measure_atoms", [])) >= needed:
            self._measure_status.set(f"{mode}: selection complete. Press Clear to start again.")
            self._measure_readout.set(
                f"{mode}\nSelection complete\nPress Clear to start again."
            )
            return
        if idx in self._measure_atoms:
            picked = ", ".join(self._atom_name(i) for i in self._measure_atoms)
            self._measure_readout.set(
                f"{mode}\nSelected: {picked}\nThat atom is already selected."
            )
            return
        self._measure_atoms.append(idx)
        self._add_measure_marker(idx)
        if len(self._measure_atoms) < needed:
            picked = ", ".join(self._atom_name(i) for i in self._measure_atoms)
            msg = f"{mode}: picked {len(self._measure_atoms)}/{needed}. Next atom..."
            self._measure_status.set(msg)
            self._measure_readout.set(f"{mode}\nSelected: {picked}\nClick atom {len(self._measure_atoms) + 1} of {needed}")
        else:
            indices = list(self._measure_atoms)
            self._draw_measurement(indices, mode)
        self._canvas_3d.draw_idle()

    def _measure_bond(self, i: int, j: int):
        self._clear_measurement(redraw=False)
        self._add_measure_marker(i)
        self._add_measure_marker(j)
        self._draw_measurement([i, j], "Distance")
        self._canvas_3d.draw_idle()

    def _nearest_3d_atom_from_click(self, event, tolerance_px: float = 46.0) -> Optional[int]:
        if event.inaxes is not self._ax_3d or not getattr(self, "_3d_atom_data", None):
            return None
        proj = self._ax_3d.get_proj()
        best_idx = None
        best_dist = float("inf")
        for idx, (x, y, z, _elem, _serial) in enumerate(self._3d_atom_data):
            x2, y2, _z2 = proj3d.proj_transform(x, y, z, proj)
            sx, sy = self._ax_3d.transData.transform((x2, y2))
            dist = ((sx - event.x) ** 2 + (sy - event.y) ** 2) ** 0.5
            if dist < best_dist:
                best_idx = idx
                best_dist = dist
        if best_idx is None or best_dist > tolerance_px:
            return None
        return best_idx

    def _on_3d_canvas_click(self, event):
        if getattr(event, "button", None) != 1:
            return
        toolbar = getattr(self, "_toolbar_3d", None)
        if toolbar is not None and getattr(toolbar, "mode", ""):
            return
        idx = self._nearest_3d_atom_from_click(event)
        if idx is None:
            return
        if self._measure_mode.get() == "Label":
            self._toggle_3d_atom_label(idx)
        else:
            self._handle_measure_pick(idx)

    def _toggle_3d_atom_label(self, idx: int):
    ...
    def _on_3d_atom_pick(self, event):
        """Handle atom and bond clicks in the 3D structure view."""
        if not hasattr(self, "_3d_atom_data") or not self._3d_atom_data:
            return

        bond = getattr(self, "_3d_bond_artists", {}).get(getattr(event, "artist", None))
        if bond is not None:
            if self._measure_mode.get() == "Distance":
                self._measure_bond(*bond)
            return
        return
    ...
    def _open_h_manager(self):
        ...
        tk.Button(bottom, text="Apply", bg="#1a3c6e", fg="black",
                  command=_apply, width=10).pack(side=tk.LEFT, padx=4)
        tk.Button(bottom, text="Apply & Close", bg="#2a4a00", fg="black",
    ...
    def _open_color_editor(self):
        ...
        for r, row in enumerate(_PT_ROWS):
            for c, elem in enumerate(row):
                ...
                try:
                    ...
                    fg = "black" if sum(rgb) < 384 else "black"
                except Exception:
                    fg = "black"
        ...
        bottom = tk.Frame(win)
        bottom.pack(fill=tk.X, padx=8, pady=8)
        tk.Button(bottom, text="Reset All to Defaults", bg="#aa2222", fg="black",
    ...
    def _clear_loaded_mos(self, status_text: str = "No MO data loaded.",
        ...
        if hasattr(self, "_comp_method_var"):
            self._comp_method_var.set("Auto")
            self._update_comp_method_options()
    ...
    def _on_loewdin_loaded(self, path: str, pops: dict):
        ...
        if getattr(self, "_orca_mo_path", None) == path and getattr(self, "_orca_mo_data", None) is not None:
            self._orca_mo_data["reduced_populations"] = pops
            if hasattr(self, "_comp_method_combo"):
                self._update_comp_method_options()
            if pops and hasattr(self, "_mo_tree"):
                sel = self._mo_tree.selection()
                if sel:
                    try:
                        iorb = int(sel[0])
                        self._refresh_current_mo_composition()
                    except Exception:
                        pass
        n = len(pops)
        ...
        if hasattr(self, "_orca_mo_data") and self._orca_mo_data is not None:
            try:
                comp = compute_mo_composition(self._orca_mo_data, iorb,
                                              ...
                                              method=self._current_comp_method_key())
    ...
    def _update_comp_method_options(self):
        """Refresh the composition method choices based on loaded ORCA data."""
        choices = ["Auto"]
        has_reduced = bool(
            getattr(self, "_orca_mo_data", None)
            and self._orca_mo_data.get("reduced_populations")
        )
        if has_reduced:
            choices.append("Löwdin reduced")
        choices.append("Raw C^2")

        self._comp_method_choices = choices
        if hasattr(self, "_comp_method_combo"):
            self._comp_method_combo.config(values=choices)
        if self._comp_method_var.get() not in choices:
            self._comp_method_var.set("Auto")

    def _current_comp_method_key(self) -> str:
        label = self._comp_method_var.get()
        if label.startswith("Löwdin"):
            return "reduced"
        if label.startswith("Raw"):
            return "raw"
        return "auto"

    def _cycle_comp_method(self):
        self._update_comp_method_options()
        choices = self._comp_method_choices
        if not choices:
            return
        cur = self._comp_method_var.get()
        idx = choices.index(cur) if cur in choices else 0
        self._comp_method_var.set(choices[(idx + 1) % len(choices)])
        self._refresh_current_mo_composition()

    def _refresh_current_mo_composition(self):
        if not getattr(self, "_orca_mo_data", None) or not hasattr(self, "_mo_tree"):
            return
        sel = self._mo_tree.selection()
        if not sel:
            return
    ...
    def _show_mo_composition(self, comp: dict, mo_index: int):
        ...
        self._comp_summary.config(
            text=f"MO {mo_index} ({mo_label})  |  "
                 f"{comp.get('method_label', 'Composition')}  |  "
                 f"s: {comp['total_s']:.1f}%  p: {comp['total_p']:.1f}%  "
                 f"d: {comp['total_d']:.1f}%  f: {comp['total_f']:.1f}%",
            fg="#994400" if comp.get('method_warning') else "#003366",
            font=("", 9, "bold")
        )
        ...
        if comp.get('method_warning'):
            self._comp_ao_detail.config(
                text=comp['method_warning'],
                fg="#994400"
            )
        elif summed:
    ...
    def _show_tracked_atom_table(self):
        ...
        tk.Button(ctrl, text="Refresh", bg="#1a3c6e", fg="black",
    ...
    def _show_mo_diagram_picker(self):
        ...
        tk.Button(opts_frame, text="Refresh contributor list",
                  bg="#1a6e3c", fg="black", font=("", 8, "bold"),
        ...
        tk.Button(btn_row, text="Build diagram", bg="#1a3c6e", fg="black",
    ...
    def _popout_render(self):
        ...
        tk.Button(toolbar, text="Re-render", bg="#2a4a00", fg="black",
                  font=("", 9), command=lambda: _rerender()
                  ).pack(side=tk.LEFT, padx=4, pady=2)
        tk.Button(toolbar, text="Save PNG", bg="#1a3c6e", fg="black",
                  font=("", 9), command=lambda: self._save_popout_png(pop_fig)
                  ).pack(side=tk.LEFT, padx=4, pady=2)
        tk.Button(toolbar, text="Save SVG", bg="#1a3c6e", fg="black",
                  font=("", 9), command=lambda: self._save_popout_svg(pop_fig)
                  ).pack(side=tk.LEFT, padx=4, pady=2)

        tk.Label(toolbar, text="  BG:", fg="black", bg="#2a2a2e",
        ...
        # Title
        tk.Label(text_frame, text="Title:", fg="black", bg="#333338",
        ...
        # Annotation text
        tk.Label(text_frame, text="Text:", fg="black", bg="#333338",
        ...
        # Font family
        tk.Label(text_frame, text="Font:", fg="black", bg="#333338",
        ...
        # Font size
        tk.Label(text_frame, text="Size:", fg="black", bg="#333338",
        ...
        # Font color
        tk.Label(text_frame, text="Color:", fg="black", bg="#333338",
        ...
        # Apply text button
        tk.Button(text_frame, text="Apply Text", bg="#4a4a00", fg="black",
                  font=("", 9, "bold"),
    ...
    def _build_tab_decomp(self, p):
        ...
        tk.Button(top_btn, text="  Analyse  ", bg="#002255", fg="black",
                  font=("", 10, "bold"),
...
def _write_cube_file(path: str, cube: dict) -> None:
    ...
    lines: List[str] = []
    comment = cube.get("comment", "simur cube").split("|")[0].strip()
    lines.append(comment[:78])
    lines.append("Generated by simur")
